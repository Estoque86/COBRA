...
void
AnnotatedTopologyReader::ApplyDelayMetric ()
{
  BOOST_FOREACH (const Link &link, m_linksList)
    {
      NS_LOG_DEBUG ("Delay: " << link.GetAttribute ("Delay"));
      //uint16_t metric = boost::lexical_cast<uint16_t> (link.GetAttribute ("Delay"));
      std::string metricDelay = boost::lexical_cast<std::string> (link.GetAttribute ("Delay"));

      std::string delayString = metricDelay.substr(0, metricDelay.size()-2);
      uint16_t metric = boost::lexical_cast<unsigned short>(delayString);
