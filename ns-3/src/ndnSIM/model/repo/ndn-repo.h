...
public:
   ...
   */
  Entry (Ptr<Repo> rp, Ptr<const ContentObject> header, Ptr<const Packet> packet);
...
private:
  Ptr<Repo> m_rp; ///< \brief content store to which entry is added
...
public:
  ...
  /**
   * @brief Get the maximum number of entries in content store
   */
  virtual uint32_t
  GetMaxSize () const = 0;
   ...
   */
  static inline Ptr<Repo>
  GetRepo (Ptr<Object> node);

protected:
  TracedCallback<Ptr<const Interest>,
                 Ptr<const ContentObject> > m_repoHitsTrace; ///< @brief trace of cache hits

  TracedCallback<Ptr<const Interest> > m_repoMissesTrace; ///< @brief trace of cache misses
};

inline std::ostream&
operator<< (std::ostream &os, const Repo &rp)
{
  rp.Print (os);
  return os;
}
...
inline Ptr<Repo>
Repo::GetRepo (Ptr<Object> node)
{
  return node->GetObject<Repo> ();
}
