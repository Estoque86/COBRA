...
{
  uint32_t size = 2 + ((2 + 2) + (4) + (m_name->GetSerializedSize ()) + (2 + 2 + 4 + 2 + 2 + (2 + 0)));
  if (m_signature != 0)
    size += 4;
...
ContentObject::Serialize (Buffer::Iterator start) const
{
  ...
  // Additional Info
  if (GetAdditionalInfo ().GetHopCount () != 100000)
  {
	  start.WriteU32 (GetAdditionalInfo ().GetHopCount());
  }
  // name
...
ContentObject::Deserialize (Buffer::Iterator start)
{
  ...
  // ** [MT] ** Additional Info
  GetAdditionalInfo ().SetHopCount(i.ReadU32 ());
...
ContentObject::AdditionalInfo::AdditionalInfo ()
  : m_hopCount(50000)
{
}

/*void
ContentObject::AdditionalInfo::SetSenderID (const uint32_t ID)
{
  m_nodeID = ID;
}

uint32_t
ContentObject::AdditionalInfo::GetSenderID () const
{
  return m_nodeID;
}
*/
...
void
ContentObject::AdditionalInfo::SetHopCount (const uint32_t COUNT)
{
  m_hopCount = COUNT;
}

uint32_t
ContentObjectHeader::AdditionalInfo::GetHopCount () const
{
  return m_hopCount;
}
