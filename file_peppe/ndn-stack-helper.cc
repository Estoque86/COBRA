  ...
  , m_needSetDefaultRoutes (false)
{
  ...
  m_repoFactory.        SetTypeId ("ns3::ndn::rp::Persistent");      // It gives the possibility to set-up a repo per each node
  m_bfFibFactory.       SetTypeId ("ns3::ndn::BloomFilterBase");      // It gives the possibility to set-up a repo per each node
  m_bfCacheFactory.       SetTypeId ("ns3::ndn::BloomFilterBase");      // It gives the possibility to set-up a repo per each node
...
void
StackHelper::SetRepository (const std::string &repository,
                                  const std::string &attr1, const std::string &value1,
                                  const std::string &attr2, const std::string &value2,
                                  const std::string &attr3, const std::string &value3,
                                  const std::string &attr4, const std::string &value4)
{
  m_repoFactory.SetTypeId (repository);
  if (attr1 != "")
      m_repoFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
      m_repoFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
      m_repoFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
      m_repoFactory.Set (attr4, StringValue (value4));
}


void
StackHelper::SetBfFib (const std::string &bloom,
                                  const std::string &attr1, const std::string &value1,
                                  const std::string &attr2, const std::string &value2,
                                  const std::string &attr3, const std::string &value3,
                                  const std::string &attr4, const std::string &value4,
...
{
  m_bfFibFactory.SetTypeId (bloom);
  if (attr1 != "")
      m_bfFibFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
	  m_bfFibFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
	  m_bfFibFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
	  m_bfFibFactory.Set (attr4, StringValue (value4));
  if (attr5 != "")
      m_bfFibFactory.Set (attr5, StringValue (value5));
  if (attr6 != "")
	  m_bfFibFactory.Set (attr6, StringValue (value6));
  if (attr7 != "")
	  m_bfFibFactory.Set (attr7, StringValue (value7));
}


void
StackHelper::SetBfCache (const std::string &bloom,
                                  const std::string &attr1, const std::string &value1,
                                  const std::string &attr2, const std::string &value2,
                                  const std::string &attr3, const std::string &value3,
                                  const std::string &attr4, const std::string &value4,
                                  ...
                                  const std::string &attr7, const std::string &value7)
{
  m_bfCacheFactory.SetTypeId (bloom);
  if (attr1 != "")
      m_bfCacheFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
	  m_bfCacheFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
	  m_bfCacheFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
	  m_bfCacheFactory.Set (attr4, StringValue (value4));
  if (attr5 != "")
      m_bfCacheFactory.Set (attr5, StringValue (value5));
  if (attr6 != "")
	  m_bfCacheFactory.Set (attr6, StringValue (value6));
  if (attr7 != "")
	  m_bfCacheFactory.Set (attr7, StringValue (value7));
}
...
StackHelper::Install (Ptr<Node> node) const
{
  ...
  // ** [MT] ** Create and aggregate Bloom Filter
  std::stringstream str_m;
  str_m << node->GetNDevices();         // In this implementation there will be allocated as many BFs as the interfaces of the node.
  const std::string devices = str_m.str();

  //m_bfFibFactory.Set("NodeInterfaces", StringValue(devices));
  //ndn->AggregateObject(CreateObjectWithAttributes<BloomFilterContainer>("NodeInterfaces", StringValue(devices)));
  //ndn->AggregateObject(m_bfFactory.Create<BloomFilterBase> ());
  ...
  Ptr<ForwardingStrategy> fw = m_strategyFactory.Create<ForwardingStrategy> ();

  StringValue bloomScope;
  UintegerValue simType;

  std::string bf_sc = "g_scopeOfBf";
  std::string simT = "g_simType";

  ns3::GlobalValue::GetValueByNameFailSafe(bf_sc, bloomScope);
  ns3::GlobalValue::GetValueByNameFailSafe(simT, simType);

  std::string blScope = bloomScope.Get();
  uint32_t sim = simType.Get();

  if (sim == 3)
  {
	  Ptr<BloomFilterBase> bfFib;
	  Ptr<BloomFilterBase> bfCache;

	  if(blScope.compare("fib") == 0)
	  {
		  bfFib = m_bfFibFactory.Create<BloomFilterBase>();

		  fw->SetBfFib(bfFib);

		  bfFib->SetForwardingStrategy(fw);

		  bfFib->SetL3Protocol(ndn);
	  }

	  else if(blScope.compare("cache") == 0)
	  {
		  bfCache = m_bfCacheFactory.Create<BloomFilterBase>();

		  fw->SetBfCache(bfCache);

		  bfCache->SetForwardingStrategy(fw);

		  bfCache->SetL3Protocol(ndn);
	  }
	  else
	  {
		  bfFib = m_bfFibFactory.Create<BloomFilterBase>();
  ...
  ndn->AggregateObject (fw);

  // Create and aggregate content store
  ndn->AggregateObject (m_contentStoreFactory.Create<ContentStore> ());

  // ** [MT] ** Create and aggregate REPOSITORY
  ndn->AggregateObject (m_repoFactory.Create<Repo> ());

  // Aggregate L3Protocol on node
  node->AggregateObject (ndn);

  // ** [MT] ** A different Seed is set per each node in order to differentiate the 'p' random position that are decremented at every insertion in the SBF
  if (sim == 3)
  {
	  Ptr<BloomFilterBase> bf_point = node->GetObject<ForwardingStrategy> ()->GetBfFib();
      bf_point->randomSeed = (unsigned)node->GetId();
      std::srand(bf_point->randomSeed);
  }
