...
#ifndef NDN_CONSUMER_RTX_ZIPF_H
#define NDN_CONSUMER_RTX_ZIPF_H
 ...
 */
class ConsumerRtxZipf: public App
{
public:
...
//  OnTimeout (uint32_t sequenceNumber);
  ...
  virtual void
  OnTimeout (std::vector<uint32_t>& sequenceNumber);
...
protected:
  ...
  std::map<std::string, std::vector<uint32_t>* > *seq_contenuto;    ///< @brief map the content name to the sequence number.

  uint32_t m_maxNumRtx;       ///< @brief maximum number of allowed retransmission.
  std::map< std::vector<uint32_t>*, uint32_t> *num_rtx;  ///< @brief map the sequence number of the content to the relative number of retransmission.

  std::map< std::vector<uint32_t>*, uint32_t> *contentInfoSeqNum;  ///< @brief map the content info to the sequencial sequence number.
  ...
  struct RetxSeqsContainer :
    public std::set<std::vector<uint32_t>* > { };
  ...
  struct SeqTimeout
  {
    SeqTimeout (std::vector<uint32_t>* _seq, Time _time) : seq (_seq), time (_time) { }
  ...
  struct SeqTimeoutsContainer :
    ...
    boost::multi_index::indexed_by<
      boost::multi_index::ordered_unique<
        ...
        boost::multi_index::member<SeqTimeout, std::vector<uint32_t>*, &SeqTimeout::seq>
        >,
