...
#include "/media/DATI/tortelli/Simulazioni_BF_SpecialIssue/ndnSIM_Routing_NewBf/ns-3/src/ndnSIM/model/bloom-filter/ndn-bloom-filter-base.h"
#include "/media/DATI/tortelli/Simulazioni_BF_SpecialIssue/ndnSIM_Routing_NewBf/ns-3/src/ndnSIM/model/repo/ndn-repo.h"
#include <map>
...
public:
  ...
  void
  SetBfFib(const Ptr<BloomFilterBase> ptrBf);

  void
  SetBfCache(const Ptr<BloomFilterBase> ptrBf);
  ...
  void
  SetNodeType(const std::string type);
...
protected:
   ...
   */
  virtual bool
  WillSendOutInterest (Ptr<Face> outgoingFace,
                       Ptr<const Interest> header,
                       Ptr<pit::Entry> pitEntry);
  ...
  virtual void
  PropagateInterest_NEW (Ptr<Face> inFace,
                     Ptr<const Interest> header,
                     Ptr<const Packet> origPacket,
                     Ptr<pit::Entry> pitEntry);
  ...
  /**
   * \brief It lets the positioning at the specified line inside the considered file.
   */
...
protected:
  ...
  Ptr<Repo> m_repository;

  Ptr<BloomFilterBase> m_bloomFilterFib; // Reference to BF to which this forwarding strategy is associated
  Ptr<BloomFilterBase> m_bloomFilterCache; // Reference to BF to which this forwarding strategy is associated
...
//                 Ptr<const Face> > m_inInterests; ///< @brief trace of incoming Interests
  ...
  TracedCallback< > m_dropInterestFailure; ///< @brief Dropped Interest Tracing due to Failure
